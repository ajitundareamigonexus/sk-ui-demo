...
import { MapPin, RotateCcw, Navigation, Plane, AlertCircle, Package, Car, ShieldCheck, Clock, Building2 } from 'lucide-react';
...
export default function BookingTabs({ variant = 'demo2' }: { variant?: 'demo2' | 'demo3' }) {
  ...
  const [hotelErrors, setHotelErrors] = useState<{ name?: boolean; mobile?: boolean; checkin?: boolean; checkout?: boolean }>({});
  ...
  const handleHotelEnquiry = () => {
    const errors: { name?: boolean; mobile?: boolean; checkin?: boolean; checkout?: boolean } = {};
    if (!pkgName.trim()) errors.name = true;
    if (!/^[6-9]\d{9}$/.test(pkgMobile)) errors.mobile = true;
    if (!hotelCheckIn) errors.checkin = true;
    if (!hotelCheckOut) errors.checkout = true;
    ...
    // Build WhatsApp message
    const msg = encodeURIComponent(
      `Hi Shree Krushna Travels! I'd like to enquire about a Hotel Booking.\n\n` +
      ...
      `👤 Name: ${pkgName}\n` +
      `📞 Mobile: ${pkgMobile}\n\n` +
      `Please share the best options and pricing details.`
    );
    const whatsappUrl = `https://wa.me/918796807060?text=${msg}`;
    window.open(whatsappUrl, '_blank');
  ...
  return (
    <>
      ...
      <div className={`w-full rounded-[1.25rem] p-4 sm:p-5 lg:p-4 relative shadow-xl flex flex-col gap-4 ${variant === 'demo3'
        ...
        {isPackages && (
          <div className="space-y-1 mb-2 relative z-[60]">
            {pkgSuccess ? (
          ...
          <div className="space-y-1 mb-2 relative z-[60]">
            {hotelSuccess ? (
              <div className="flex flex-col items-center justify-center py-6 space-y-3 text-center">
                ...
                <p className="text-teal-400 font-bold text-sm">Enquiry Sent via WhatsApp!</p>
                <p className="text-white/60 text-xs max-w-[200px]">
                  Our team will contact you shortly with the best hotel deals.
                </p>
                <button
                  onClick={() => setHotelSuccess(false)}
                  className="text-xs text-white/60 underline hover:text-white transition-colors"
                >
                  Submit another enquiry
                </button>
              ...
              <>
                <div className="flex flex-col gap-4">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:flex gap-4 w-full">
                    ...
                    <div className="w-full lg:flex-1">
                      <label className="block text-[11px] font-semibold text-slate-900 mb-1">Check-out *</label>
                      <input
                        type="date"
                    ...
                    <div className="w-full lg:flex-1">
                      <label className="block text-[11px] font-semibold text-slate-900 mb-1">Rooms</label>
                      <select
                        value={hotelRooms}
                    ...
                    <div className="w-full lg:flex-1">
                      <label className="block text-[11px] font-semibold text-slate-900 mb-1">Guests</label>
                      <select
                        value={hotelGuests}
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:flex gap-3 w-full">
                    ...
                    <div className="w-full lg:flex-1 lg:max-w-[250px]">
                      <label className="block text-[11px] font-semibold text-slate-900 mb-1">Mobile *</label>
                      <input
                        type="tel"
        ...
        {/* ── Cab Booking Fields ── */}
        {mainTab === 'cab' && (
          <div className="flex flex-col lg:flex-row lg:flex-wrap items-stretch lg:items-end gap-4 w-full relative z-[60]">
            ...
            {/* Date & Time Grid */}
            <div className={`grid grid-cols-2 sm:grid-cols-4 lg:flex ${isRound ? 'lg:flex-[2]' : 'lg:flex-1'} gap-4 w-full`}>
              <div className="w-full flex-1">

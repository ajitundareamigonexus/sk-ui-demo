...
export default function DomesticPackages({
  ...
  useEffect(() => {
    ...
    Promise.all([fetchSheet, fetchBackend]).then(([sheetData, backendData]) => {
      const sheetDomestic = Array.isArray(sheetData)
        ? sheetData
          .filter((item: any) => item.category === "domestic")
          .map((item: any, index: number) => ({
            ...item,
            id: item.id ? String(item.id) : `d-${index}`,
            image: item.image === "image-link" || !item.image ? "https://images.unsplash.com/photo-1566228015668-4c45dbc4e2f5?q=80&w=800" : item.image,
      ...
      const backendDomestic = Array.isArray(backendData)
        ? backendData
          .filter((item: any) => item.category === "domestic")
          .map((v: any) => ({
            id: v.id,
            ...
            image: v.image || "https://images.unsplash.com/photo-1566228015668-4c45dbc4e2f5?q=80&w=800",
      ...
      setPackages([...backendDomestic, ...sheetDomestic]);
    });
  }, []);
      ...
      {hasMore && (
        ...
        >
          ...
          <div className="absolute inset-0 z-0">
            <Image
              src={packages[5]?.image || "https://images.unsplash.com/photo-1566228015668-4c45dbc4e2f5?q=80&w=800"}
              alt="More Packages"
              fill
